...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [lead, audit] = await Promise.all([getDbLead(id, organizationId), getLatestLeadIntelligence(id)]);
  if (!lead) return NextResponse.json({ error: "Lead not found" }, { status: 404 });
  if (!audit) return NextResponse.json({ error: "Run the website audit before downloading its PDF." }, { status: 404 });

  const pdf = await buildWebsiteAuditPdf(lead, audit, { brandName: user.organization?.companyName });
