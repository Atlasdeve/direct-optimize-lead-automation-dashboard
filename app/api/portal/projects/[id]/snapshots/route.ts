import { NextRequest, NextResponse } from "next/server";
import { currentUser } from "@/lib/auth";
import { createSnapshot, getProjectForUser } from "@/lib/portalStore";

function screenshotUrls(value: unknown) {
  if (Array.isArray(value)) return value.map(String).map((item) => item.trim()).filter(Boolean).slice(0, 8);
  if (typeof value === "string") return value.split(/\n|,/).map((item) => item.trim()).filter(Boolean).slice(0, 8);
  return [];
}

export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const user = await currentUser();
  if (!user || user.role === "client") return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const { id } = await params;
  const project = await getProjectForUser(id, user);
  if (!project) return NextResponse.json({ error: "Project not found" }, { status: 404 });
  const body = await request.json().catch(() => ({}));
  try {
    return NextResponse.json({
      project: await createSnapshot(id, {
        kind: String(body.kind || "Before"),
        title: String(body.title || "Project snapshot"),
        summary: String(body.summary || ""),
        metrics: typeof body.metrics === "object" && body.metrics ? body.metrics : {},
        screenshotUrls: screenshotUrls(body.screenshotUrls)
      })
    });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Snapshot could not be saved." }, { status: 400 });
  }
}
